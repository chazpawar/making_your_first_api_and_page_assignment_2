...
// Endpoint to get HTTP status code details
app.get('/status-info', (req, res) => {
    ...
    if (message) {
        ...
        res.status(404).json({
            status: 404,
            message: `Not Found: The provided status code (${code}) is not recognized. Supported status codes are: ${Object.keys(statusDescriptions).join(', ')}.`
        });
    }
});

// Endpoint for the virtual assistant greeting
